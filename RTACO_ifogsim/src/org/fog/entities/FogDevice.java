...
public class FogDevice extends PowerDatacenter {
    ...
    protected void updateAllocatedMips(String incomingOperator) {
        getHost().getVmScheduler().deallocatePesForAllVms();  // 清空所有VM的PE分配
        for (final Vm vm : getHost().getVmList()) {  // 遍歷所有VM
            if (vm.getCloudletScheduler().runningCloudlets() > 0 || ((AppModule) vm).getName().equals(incomingOperator)) {
                // 如果有正在運行的Cloudlet，或VM的名字和傳入的operator相同
                List<Double> allocatedMips = new ArrayList<>();
                allocatedMips.add((double) getHost().getTotalMips());  // 分配所有的MIPS
                getHost().getVmScheduler().allocatePesForVm(vm, allocatedMips);
            } else {
                List<Double> allocatedMips = new ArrayList<>();
                allocatedMips.add(0.0);  // 如果不符合條件，分配0 MIPS
                getHost().getVmScheduler().allocatePesForVm(vm, allocatedMips);
            }
        }

        updateEnergyConsumption();  // 更新能源消耗
    } 
